        ...
        **/
        var _abrirRelatorio = function () {
            var printContents = document.getElementById("Relatorio_Impressao").innerHTML,                         
                ...
                html = Prion.Base64.encode(html);
               
                Prion.Request({
                    url: rootDir + "Relatorios/GerarPDF",
                    data: "Html=" + html + "&NomeRelatorio=" + tipoRelatorio,
                    success: function (retorno) {
                        window.alert("O PDF está sendo criado.Aguarde por favor");
                        _prepararBotaoAbrirPdf(retorno.url);

                        while (!_verificaEstadoPdf(retorno.url)) { }

                        window.alert("Criação concluída!");
        ...
        **/
        var _criarRelatorioImpressao = function(jsonObj){
            ...
            for(var i=0,len = jsonObj.length;i<len;i++){
                ...
                objectKeys = Object.keys(jsonObj[i]);
