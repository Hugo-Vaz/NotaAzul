...
namespace NotaAzul.Controllers
{
    public class MatriculaController : BaseController
    {
        ...
        public ActionResult Salvar(ViewModels.Matricula.vmDados vModel)
        {
            Prion.Generic.Helpers.Mensagem mensagem = new Prion.Generic.Helpers.Mensagem();
            Business.Matricula biMatricula = new Business.Matricula();
            List<Models.Boleto> boletos = new List<Models.Boleto>();
            List<Models.MatriculaCurso> listaMatriculaCurso = JsonMatriculaCurso(Request.Form["MatriculaCurso"], vModel.Matricula, ref boletos);
            ...
            if (!biMatricula.ChecarPossibilidadeDeMatricula(anoLetivo, vModel.Matricula.IdAluno))
            {             
               
                mensagem.Tipo = GenericHelpers.TipoMensagem.Erro;
                mensagem.TextoMensagem = "Já existe uma matrícula em um curso curricular para esse Aluno.";
            ...
            GenericHelpers.Retorno retorno = biMatricula.Salvar(vModel.Matricula, boletos);
        ...
        /// <param name="vModel"></param>
        /// <returns></returns>
        public ActionResult GerarBoletos(ViewModels.Matricula.vmDados vModel)
        {
            Prion.Generic.Helpers.Mensagem mensagem = new Prion.Generic.Helpers.Mensagem();
            ...
            vModel.Matricula.ListaMatriculaCurso = listaMatriculaCurso;
            GenericHelpers.Retorno retorno = biBoleto.Salvar(boletos);
        ...
        private List<Models.MatriculaCurso> JsonMatriculaCurso(String json, Models.Matricula matricula,ref List<Models.Boleto> boletos)
        {
            ...
            Int32 quantidadeBoletos = Convert.ToInt32(Sistema.GetConfiguracaoSistema("boleto:nosso_numero")); //biBoleto.BuscarQuantidadeBoleto() + 10;
            Models.Usuario user = (Models.Usuario)Session["UsuarioLogado"];
            /**
            ...
            for (Int32 i = 0; i < result.MatriculaCurso.Count; i++)
            {
                ...
                for (Int32 j = 0; j < result.MatriculaCurso[i]["Mensalidades"].Count; j++)
                {
                    ...
                    #region "Criando Boletos"
                    if(!todosBoletosLidos)
                    {
                        if(mensalidade.DataVencimento <= DateTime.Now)
                        {
                            matriculaCurso.Mensalidades.Add(mensalidade);
                            continue;
                        }

                        if(mensalidade.DataVencimento.Month == 12)
                        {
                            todosBoletosLidos = true;
                        }

                        Models.Boleto boleto = new Models.Boleto();                        

                        boleto.EstadoObjeto = Prion.Generic.Helpers.Enums.EstadoObjeto.Novo;
                        boleto.IdCorporacao = user.IdCorporacao;
                        boleto.DataVencimento = mensalidade.DataVencimento;
                        boleto.Desconto = mensalidade.Desconto;
                        boleto.Valor = mensalidade.Valor;
                        boleto.Juros = mensalidade.Acrescimo;
                        boleto.NumeroAgencia = Helpers.Settings.CedenteAgencia();
                        boleto.CodigoBanco = Helpers.Settings.CedenteDigitoVerificador();
                        boleto.NumeroConta = Helpers.Settings.CedenteConta();
                        boleto.Convenio = Helpers.Settings.CedenteConvenio();
                        boleto.Carteira = Conversor.ToString(boleto.Convenio).PadLeft(2, '0');
                        boleto.StatusBoleto = "Aberto";
                        boleto.DataEmissao = Helpers.DataHora.GetCurrentNow();
                        boleto.DataVencimento = mensalidade.DataVencimento;
                        quantidadeBoletos = quantidadeBoletos + 1;
                        boleto.NossoNumero = Conversor.ToString(quantidadeBoletos).PadLeft(11, '0');
                        boleto.NumeroDocumento = boleto.NossoNumero;

                        if(mensalidade.Id != 0)
                        {
                            boleto.IdsMensalidade = new List<int>();
                            boleto.IdsMensalidade.Add(mensalidade.Id);
                        }
                    ...
                    else
                    {
                        ...
                        boletos[j - (12 - boletos.Count)].Valor += mensalidade.Valor;                      

                        if (mensalidade.Id != 0)
                        {
                            boletos[j - (12 - boletos.Count)].IdsMensalidade.Add(mensalidade.Id);
                        }
                    }
                    #endregion "Criando Boletos"
            ...
            config.Atributo = "boleto:nosso_numero";
            config.Valor = quantidadeBoletos.ToString();
            config.EstadoObjeto = GenericHelpers.Enums.EstadoObjeto.Alterado;
            configuracoes.Add(config);

            biCOnfig.Salvar(configuracoes);
